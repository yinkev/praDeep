...
test.describe("Learning Analytics Dashboard", () => {
  test("should load the analytics page and display main components", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Learning Analytics|学习分析/i }),
    ).toBeVisible();

    // Check that time range filter is visible
    await expect(
      page.getByRole("button", { name: /Today|今天/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /This Week|本周/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /This Month|本月/i }),
    ).toBeVisible();
    ...
    await page.waitForSelector("text=/Total Activities|总活动数/i", {
      timeout: 30000,
    });

    // Check that summary cards are displayed
    await expect(
      page.locator("text=/Total Activities|总活动数/i"),
    ).toBeVisible();
  ...
  test("should display learning scores section", async ({ page }) => {
    ...
    await page.waitForSelector("text=/Learning Scores|学习评分/i", {
      timeout: 30000,
    });

    // Check that Learning Scores section is visible
    await expect(
      page.locator("text=/Learning Scores|学习评分/i"),
    ).toBeVisible();

    // Check for score labels
    await expect(page.locator("text=/Overall|综合/i").first()).toBeVisible();
    await expect(
      page.locator("text=/Engagement|参与度/i").first(),
    ).toBeVisible();
    await expect(
      page.locator("text=/Consistency|持续性/i").first(),
    ).toBeVisible();
    await expect(
      page.locator("text=/Diversity|多样性/i").first(),
    ).toBeVisible();
  });

  test("should display strength areas and knowledge gaps sections", async ({
    page,
  }) => {
    await page.goto("/analytics");
    await page.waitForSelector("h1", { timeout: 60000 });
    ...
    // Wait for data to load
    await page.waitForSelector("text=/Strength Areas|优势领域/i", {
      timeout: 30000,
    });
    ...
    await expect(
      page.locator("text=/Areas to Review|需要复习的领域/i"),
    ).toBeVisible();
  });
