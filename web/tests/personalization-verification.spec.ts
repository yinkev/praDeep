...
test.describe("Personalization page", () => {
  test("renders sections and runs personalization flows", async ({ page }) => {
    ...
    await page.route("**/api/v1/personalization/**", async (route) => {
      ...
      if (pathname.endsWith("/api/v1/personalization/difficulty")) {
        if (request.method() === "POST")
          lastDifficultyBody = request.postDataJSON();
        await route.fulfill({
          status: 200,
          headers: corsHeaders(),
          contentType: "application/json",
      ...
      if (pathname.endsWith("/api/v1/personalization/learning-path")) {
        if (request.method() === "POST")
          lastLearningPathBody = request.postDataJSON();
        await route.fulfill({
          status: 200,
          headers: corsHeaders(),
          contentType: "application/json",
            ...
            milestones: [
              {
                ...
                description:
                  "Tighten calculus, linear algebra, and probability essentials.",
                estimated_hours: 8,
                resources: [
                  "Khan Academy notes",
                ...
                activities: [
                  "Derive backprop for a 2-layer MLP",
                  "Implement SGD from scratch",
                ],
                success_criteria: [
                  "Can compute gradients for common layers without referencing notes.",
                ],
              },
              {
                title: "Modern architectures",
                description:
                  "Transformers, normalization, attention patterns, and optimization.",
                estimated_hours: 18,
                resources: [
                ...
                success_criteria: [
                  "Can explain attention scaling + failure modes.",
                ],
              },
              {
                title: "Research & production habits",
                description:
                  "Reading papers, ablations, evals, and deployment constraints.",
                estimated_hours: 16,
                resources: ["Papers With Code", "MLOps basics"],
                activities: [
                  "Reproduce a paper figure",
                  "Write an ablation plan",
                ],
                success_criteria: [
                  "Can design an eval suite and justify metric choices.",
                ],
              },
            ],
            tips: [
              "Keep sessions to 45–60 minutes with short breaks.",
              "Ship tiny demos weekly.",
            ],
          }),
        });
        return;
      ...
      if (pathname.endsWith("/api/v1/personalization/behavior-insights")) {
        await route.fulfill({
          ...
          body: JSON.stringify({
            ...
            strengths: [
              "Deep focus in longer sessions",
              "Strong follow-through on milestones",
            ],
            improvement_areas: [
              "Context switching between topics",
              "Starting sessions without clear goals",
            ],
            patterns: {
              consistency: "Most productive on Tue–Thu; weekends are lighter.",
              focus_duration:
                "Peak focus window ~52 minutes before attention drops.",
              topic_diversity:
                "Moderate: tends to batch related topics in sprints.",
            },
            ...
            recommendations: [
              {
                category: "Routine",
                suggestion:
                  "Add a 5-minute daily review to prime the next session.",
                impact: "high",
              },
              {
                category: "Focus",
                suggestion:
                  "Use 50/10 cycles and end sessions with a written next step.",
                impact: "medium",
              },
            ],
            next_steps: [
              "Pick one focus ritual to trial for 7 days.",
              "Reduce topic switches to <2/day.",
            ],
          }),
        });
        return;
    ...
    await expect(
      page.getByRole("heading", { name: "Personalization" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Learning Style Detection" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Difficulty Calibration" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Learning Path Generator" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Behavior Insights" }),
    ).toBeVisible();

    await page.getByRole("button", { name: /^Detect style$/i }).click();
    const learningStyleRegion = page.getByRole("region", {
      name: "Learning Style Detection",
    });
    await expect(
      learningStyleRegion.getByText("Detected: Visual"),
    ).toBeVisible();

    await page.getByLabel("Calibration topic").fill("Linear algebra");
    await page.getByRole("button", { name: /^Calibrate$/i }).click();
    const difficultyRegion = page.getByRole("region", {
    ...
    expect(lastLearningPathBody).toEqual({
      topic: "Neural networks",
      target_level: "advanced",
    });
